...
def check_federal_filings(seen):
    ...
    for item in data.get("results", []):
        ...
        # Get opinion text for AI summary
        snippet = re.sub(r'<[^>]+>','',item.get("snippet","") or "").strip()
        opinion_text = snippet
        if item.get("plain_text") and len(item["plain_text"]) > 100:
            opinion_text = item["plain_text"][:10000]
        elif item.get("html") and len(item.get("html","")) > 100:
            opinion_text = re.sub(r'<[^>]+>','',item["html"])[:10000]
        stxt = f"Court Opinion in: {cn}\n\n{opinion_text}"
        summary = summarize_with_deepseek(stxt, "federal court opinion") if len(stxt) > 80 else "Limited details available."
...
def check_federal_company(seen, idx):
    """Search for opinions mentioning specific companies from the full SEC database."""
    # Build search list from ALL SEC-registered companies (not just 100)
    # Use the PUBLIC_COMPANIES dict keys (company names), sorted by length desc for most specific first
    all_names = sorted(set(PUBLIC_COMPANIES.keys()), key=len, reverse=True)
    # Filter to names 5+ chars to avoid garbage searches, skip blocklisted short names
    searchable = [n for n in all_names if len(n) >= 5]
    if not searchable:
        return 0
    co = searchable[idx % len(searchable)]
    yesterday = (datetime.now(EST) - timedelta(days=1)).strftime("%m/%d/%Y")
    ...
    for item in data.get("results",[]):
        ...
        if item.get("plain_text") and len(item["plain_text"]) > 100:
            ...
            opinion_text = re.sub(r'<[^>]+>','',item.get("snippet","") or "").strip()
...
def check_high_impact_filings(seen):
    """Search for high-impact NEW filings: SEC enforcement, DOJ antitrust, class actions, injunctions.
    Uses type=r but filtered by keywords that indicate market-moving events."""
    yesterday = (datetime.now(EST) - timedelta(days=1)).strftime("%m/%d/%Y")
    # Rotate through high-impact search terms
    queries = [
        "securities fraud class action",
        "SEC enforcement action",
        "antitrust DOJ",
        "preliminary injunction patent",
        "settlement agreement securities",
        "FTC complaint",
        "securities class action certification",
        "qui tam whistleblower",
    ]
    # Pick 2 queries per cycle to keep API usage reasonable
    ...
    if not data: return 0
    alerts = 0
    for item in data.get("results", [])[:10]:  # Cap at 10 results
        cn = item.get("caseName","") or item.get("case_name","")
        ...
        matches = match_public_company(cn)
        if not matches: continue
        log.info(f"HIGH-IMPACT [{q[:20]}]: {cn[:80]}")
        snippet = re.sub(r'<[^>]+>','',item.get("snippet","") or "").strip()
        ...
        embed = {"title":f"🚨 High-Impact Filing: {cn[:200]}","url":doc_url,"color":0xFF0000,
            "fields":[
                ...
                {"name":"📌 Category","value":q.title(),"inline":True},
            ],
            "footer":{"text":f"Court Monitor | {rate_limiter.remaining()} left"},"timestamp":datetime.utcnow().isoformat()}
        if item.get("court"): embed["fields"].append({"name":"🏛️ Court","value":item["court"],"inline":True})
    ...
    try:
        ...
        for row in p.rows[-15:]:
            ...
            embed = {"title":f"🏛️ SCOTUS: {rt[:200]}",
                "url":scotus_url,"color":0xFF0000,
                "fields":[{"name":"🚨 PUBLIC COMPANY","value":tstr,"inline":False},
                    {"name":"📄 Document","value":f"[View on supremecourt.gov]({scotus_url})","inline":False},
