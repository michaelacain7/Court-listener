...
def main():
    ...
    cycle = 0; cidx = 0; didx = 0; bidx = 0; last_save = time.time()
    ...
    startup = {"title":"🟢 Court Filing Monitor Online","description":(
        f"• Polling: every **{CHECK_INTERVAL}s** (staggered across 10 sources)\n"
    ...
    while True:
        try:
            ...
            # Staggered 10-phase cycle at 10s intervals (each source every 100s)
            ...
            # After completing first full 10-phase cycle, exit seed mode
            if SEED_MODE and phase == 9:
                SEED_MODE = False
                save_seen(seen)
