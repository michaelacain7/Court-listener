...
# "V.O.S. Selections v. Trump" (tariff refunds) is a classic example.
MACRO_KEYWORDS = {
    ...
    if docket_id and rate_limiter.remaining() > 300:
        try:
            ...
            if docket_data:
                ...
                # NOS codes: https://www.uscourts.gov/sites/default/files/js_044_1.pdf
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code in (200, 204):
                # Mirror to the combined webhook (fire-and-forget)
...
def check_high_impact_filings(seen):
    ...
    yesterday = (datetime.now(EST) - timedelta(days=1)).strftime("%m/%d/%Y")
    # Rotate through high-impact search terms — institutional/govt actions + macro topics + verdicts
    queries = [
        ...
        # Jury verdicts and large awards (catches cases like Skillz v. Papaya $420M)
        '"jury verdict" damages',
        '"jury returned" verdict',
        '"judgment as a matter of law" denied',
        '"damages" million verdict',
        '"false advertising" Lanham Act verdict',
...
def check_verdicts(seen):
    """Search for jury verdicts, judgments, and large monetary awards.
    This catches cases like 'Skillz v. Papaya $420M' where a small/mid-cap public
    company wins or loses big damages. Searches type=r (RECAP) which includes
    verdict docket entries that don't appear in opinion searches (type=o).

    Uses broad keyword searches that find any verdict, then matches against
    the full SEC ticker database (8000+ companies) to catch small-caps."""

    # Look back 3 days — verdicts take a beat to get indexed on CL
    recent = (datetime.now(EST) - timedelta(days=3)).strftime("%m/%d/%Y")

    # Rotate through verdict-specific queries
    verdict_queries = [
        '"jury verdict" OR "jury found"',
        '"verdict in favor" damages',
        '"awarded damages" million',
        '"judgment" OR "final judgment" damages',
        '"summary judgment granted" OR "summary judgment denied"',
        'verdict "$" million',
        '"Lanham Act" verdict',
        '"false advertising" verdict damages',
        'punitive damages awarded jury',
        'trebled damages patent',
    ]
    cycle_idx = int(time.time() / 60) % len(verdict_queries)
    q = verdict_queries[cycle_idx]

    data = cl_request(CL_SEARCH_URL, params={
        "q": q, "type": "r", "order_by": "dateFiled desc", "filed_after": recent
    })
    if not data:
        return 0

    alerts = 0
    # Dollar amount regex — captures verdicts of material size
    ...
    for item in data.get("results", [])[:15]:
        ...
        fid = make_id(item, "vrdct")
        if fid in seen:
            continue

        # Match against full SEC ticker database (8000+ companies)
        matches = match_public_company(cn)
        snippet = re.sub(r'<[^>]+>', '', item.get("snippet", "") or "").strip()

        # Also check buzzy private companies
        if not matches:
        ...
        # Also check the snippet for company names (verdicts mention parties in description)
        if not matches and snippet:
            matches = match_public_company(snippet[:500])
        ...
        # Extract dollar amounts from snippet — prioritize large verdicts
        dollar_amounts = dollar_re.findall(snippet + " " + cn)
        dollar_str = ""
        is_large = False
        for amount, unit in dollar_amounts:
            try:
                ...
                if unit.lower().startswith("b"):
                    val *= 1000  # billions
                if val >= 10:  # >= $10M is material
                    is_large = True
                dollar_str = f"${amount} {unit}"
                break
            except ValueError:
                pass

        # Pre-filter: skip obvious individual plaintiff cases
        parts = re.split(r'\s+v\.?\s+', cn, maxsplit=1, flags=re.IGNORECASE)
        if len(parts) == 2:
            plaintiff = parts[0].strip()
            # Skip if plaintiff is a short individual name (no company/govt markers)
            company_indicators = ["inc", "llc", "corp", "ltd", "co.", "company", "group",
                                  "holdings", "partners", "fund", "bank", "platform",
                                  "technologies", "systems", "enterprises"]
            govt_patterns = ["united states", "securities and exchange", "federal trade",
                             "department of", "state of", "in re"]
            is_institutional = (any(ci in plaintiff.lower() for ci in company_indicators)
                                or any(gp in plaintiff.lower() for gp in govt_patterns))
            if not is_institutional and not is_large:
                plaintiff_words = [w for w in plaintiff.split() if len(w) > 1]
                if len(plaintiff_words) <= 3 and "," not in plaintiff:
                    log.info(f"  VERDICT pre-filter: Individual plaintiff '{plaintiff}' → SKIP")
                    continue

        log.info(f"VERDICT [{q[:30]}]: {cn[:80]} {dollar_str}")

        # Build summary
        summary_input = (
            f"Court Verdict/Judgment:\n"
            f"Case: {cn}\n"
            f"Query matched: {q}\n"
            f"Court: {item.get('court', '')}\n"
            f"Date: {item.get('dateFiled', '')}\n"
            f"Dollar amount detected: {dollar_str or 'not specified'}\n\n"
            f"{snippet}"
        )
        summary = summarize_with_deepseek(summary_input, "court verdict or judgment")

        tstr = format_tickers(matches)
        doc_url = get_filing_url(item, "r")

        # Color: bright red for large verdicts, normal red otherwise
        color = 0xDC143C if is_large else 0xFF4500
        emoji = "💰" if is_large else "⚖️"

        embed = {
            "title": f"{emoji} Verdict/Judgment: {cn[:200]}",
            "url": doc_url,
            "color": color,
            "fields": [
                {"name": "📊 Companies", "value": tstr, "inline": False},
            ],
            "footer": {"text": f"Court Monitor | Verdicts | {rate_limiter.remaining()} left"},
            "timestamp": datetime.now(tz=pytz.utc).isoformat()
        }
        if dollar_str:
            embed["fields"].append({"name": "💵 Amount", "value": dollar_str, "inline": True})
        if item.get("court"):
    ...
    try:
        ...
        for url, title, pdf_url in entries[:20]:  # Check latest 20
            ...
            if matches:
                ...
                # Macro match — show sector tags
                tstr = " | ".join(sector for _, sector in macro_matches[:3])

            embed = {
                "title": f"{'🔴' if is_macro else '🚨'} CAFC {doc_type}: {case_name[:200]}",
                "url": url,
                "color": 0xFF0000 if is_macro else 0xFF0000,
                "fields": [
                    {"name": "📊 Impact" if is_macro else "📊 Tickers", "value": tstr, "inline": False},
...
def main():
    ...
    log.info(f"  Check interval: {CHECK_INTERVAL}s (13 sources, each every {CHECK_INTERVAL*13}s)")
    ...
    startup = {"title":"🟢 Court Filing Monitor Online","description":(
        f"• Polling: every **{CHECK_INTERVAL}s** (staggered across 13 sources)\n"
        f"• Federal courts: **opinions + new dockets** (rulings AND lawsuits)\n"
        f"• 💰 Verdicts: jury awards, judgments, damage awards (all tickers)\n"
    ...
    while True:
        try:
            ...
            # Staggered 13-phase cycle at CHECK_INTERVAL intervals
            ...
            # Phase 12: Jury verdicts + judgments (catches Skillz v. Papaya type wins)
            phase = cycle % 13
            ...
            if phase in (0, 4):
                ...
                n = check_verdicts(seen)
                if n and not SEED_MODE: log.info(f"Sent {n} verdict/judgment alerts")

            # After completing first full 13-phase cycle, exit seed mode
            if SEED_MODE and phase == 12:
                SEED_MODE = False
                save_seen(seen)
