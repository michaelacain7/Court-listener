...
def check_federal_company(seen, idx):
    # Top 100 companies by market relevance for targeted API searches
    companies = [
        ...
        "JPMorgan","Goldman Sachs","Bank of America","Wells Fargo","Morgan Stanley","Citigroup","BlackRock","Blackstone",
        ...
        "UnitedHealth","CVS Health","Humana","Cigna","HCA Healthcare",
        "Visa","Mastercard","American Express","PayPal","Coinbase",
        "AT&T","Verizon","T-Mobile","Charter Communications",
        "Uber","Airbnb","Booking Holdings","Expedia",
        "Qualcomm","Texas Instruments","Applied Materials","Micron",
        "IBM","Cisco","Oracle","Salesforce","Adobe","ServiceNow",
        "Caterpillar","Deere","Honeywell","General Electric","3M",
        "FedEx","UPS","Union Pacific","Norfolk Southern","CSX",
        "Ford","General Motors","Rivian","Lucid",
        ...
        "CrowdStrike","Palantir","Snowflake","Shopify","Spotify",
        "Berkshire Hathaway","Robinhood","MicroStrategy","Affirm","SoFi",
    ]
...
def check_cafc_website(seen):
    """Scrape CAFC opinions/orders page directly for new filings involving public companies."""
    alerts = 0
    CAFC_URL = "https://www.cafc.uscourts.gov/category/opinion-order/"
    CAFC_BASE = "https://www.cafc.uscourts.gov"
    try:
        resp = requests.get(CAFC_URL, timeout=15,
            headers={"User-Agent": "CourtFilingMonitor/1.0"})
        if not resp.ok:
            log.warning(f"CAFC page returned {resp.status_code}")
            return 0

        # Parse blog-style posts: each has <h2 class="entry-title"><a href="...">CASE NAME [TYPE]</a></h2>
        # Also try <article> or <a> patterns with case names
        import re as _re

        # Pattern 1: WordPress post titles (most reliable)
        # Match: <a href="https://www.cafc.uscourts.gov/MM-DD-YYYY-.../">CASE NAME [TYPE]</a>
        post_pattern = _re.compile(
            r'<a\s+href="(https?://www\.cafc\.uscourts\.gov/\d{2}-\d{2}-\d{4}[^"]*)"[^>]*>\s*'
            r'([^<]+?)\s*</a>', _re.IGNORECASE)

        # Pattern 2: PDF links from the opinions table
        ...
        entries = []
        for match in post_pattern.finditer(resp.text):
        ...
        for url, title in entries[:20]:  # Check latest 20
            ...
            # Try to fetch PDF text for better AI summary
            summary_text = f"Federal Circuit (CAFC) Opinion/Order:\n{case_name}\nURL: {url}"
            if url.endswith('.pdf'):
                try:
                    pdf_resp = requests.get(url, timeout=20,
                        headers={"User-Agent": "CourtFilingMonitor/1.0"})
                    if pdf_resp.ok and len(pdf_resp.content) < 500000:  # <500KB
                        # Extract text from first few pages via simple pattern
                        pdf_text = pdf_resp.content.decode('utf-8', errors='ignore')
                        # Get readable portions
                        readable = _re.sub(r'[^\x20-\x7E\n]', ' ', pdf_text)
                        readable = _re.sub(r'\s+', ' ', readable).strip()
                        if len(readable) > 200:
                            summary_text = f"CAFC Opinion:\n{case_name}\n\n{readable[:8000]}"
                except Exception as e:
                    log.debug(f"CAFC PDF fetch failed: {e}")
            ...
            embed = {
                "title": f"⚖️ CAFC {doc_type}: {case_name[:200]}",
                "url": url,
                "color": 0x8B0000,  # Dark red for CAFC
                "fields": [
...
def main():
    ...
    log.info(f"  Check interval: {CHECK_INTERVAL}s (7 sources, each every {CHECK_INTERVAL*7}s)")
    ...
    startup = {"title":"🟢 Court Filing Monitor Online","description":(
        f"• Polling: every **{CHECK_INTERVAL}s** (staggered across sources)\n"
        f"• Federal courts: broad search + targeted company search\n"
        f"• Supreme Court: opinions + dockets + supremecourt.gov\n"
        f"• **CAFC: direct scraping** (patent appeals, most market-moving)\n"
        f"• Companies: **{unique_tickers}** tickers ({len(PUBLIC_COMPANIES)} name variants) — ALL SEC-registered\n"
    ...
    while True:
        try:
            ...
            # Staggered 7-phase cycle: each source checked every 70s at 10s intervals
            # Phase 0: Federal broad     Phase 1: SCOTUS opinions (CL API)
            # Phase 2: Federal company   Phase 3: SCOTUS docket (CL API)
            # Phase 4: Federal broad     Phase 5: SCOTUS website (no API)
            # Phase 6: CAFC website (no API)
            phase = cycle % 7
