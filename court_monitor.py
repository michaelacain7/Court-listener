...
def check_delaware_courts(seen):
    ...
    alerts = 0
    for court_name, court_param in [("court+of+chancery", "Chancery"), ("supreme+court", "DE Supreme")]:
        url = f"https://courts.delaware.gov/opinions/index.aspx?ag={court_param.lower().replace(' ','+')}"
        if court_param == "Chancery":
        ...
        try:
            ...
            if resp.status_code != 200:
                log.debug(f"Delaware {court_param} returned {resp.status_code}")
                continue
            html = resp.text
        except Exception as e:
            log.debug(f"Delaware {court_param} fetch failed: {e}")
            continue
        ...
        for pdf_path, case_name, date_str in rows[:20]:
            ...
            log.info(f"DE {court_param}: {case_name[:80]}")
            pdf_url = f"https://courts.delaware.gov{pdf_path}"

            # Try to fetch opinion PDF text for AI summary
            opinion_text = ""
            try:
                pdf_resp = requests.get(pdf_url, timeout=20, headers={
                    "User-Agent": "Mozilla/5.0 (compatible; CourtMonitor/1.0)"
                })
                if pdf_resp.status_code == 200 and len(pdf_resp.content) > 500:
                    ...
                    try:
                        ...
                        opinion_text = f"Delaware {court_param} opinion: {case_name}"
            except Exception as e:
                log.debug(f"DE PDF fetch failed: {e}")

            if not opinion_text:
                opinion_text = f"Delaware {court_param} opinion in: {case_name}. Filed {date_str}."

            summary = summarize_with_deepseek(opinion_text, f"Delaware {court_param} opinion")
            tstr = format_tickers(matches)

            embed = {"title": f"🏛️ DE {court_param}: {case_name[:200]}", "url": pdf_url, "color": 0xFF4500,
                "fields": [{"name": "📊 Companies", "value": tstr, "inline": False}],
                "footer": {"text": f"Court Monitor | Delaware State Courts"},
                "timestamp": datetime.utcnow().isoformat()}
            embed["fields"].append({"name": "🏛️ Court", "value": f"Delaware {court_param}", "inline": True})
...
def main():
    ...
    cycle = 0; cidx = 0; didx = 0; bidx = 0; sidx = 0; last_save = time.time()
    ...
    startup = {"title":"🟢 Court Filing Monitor Online","description":(
        ...
        f"• ⚡ Docket watch: new lawsuits involving major companies\n"
    ...
    while True:
        try:
            ...
            # Phase 11: Federal opinions (broad) - extra pass
            phase = cycle % 12
